...
class PollerConfigParserTest {
    ...
    fun testParse_validation_maxPutDelay_zero() {
        ...
        MoreAssertions.assertThatThrownBy { PollerConfigParser.parse(toml) }
    ...
    fun testParse_validation_maxPutDelay_atLowerAndUpperBounds(value: Int) {
        ...
        val log = PollerConfigParser.parse(toml).getValue("g").log
    ...
    fun testParse_validation_logFiles_missingPattern() {
        ...
        assertThatThrownBy { PollerConfigParser.parse(toml) }
            .hasMessageContainingAll("Field 'pattern' is required for type with serial name", "FilesConfig")
    }
    ...
    fun testParse_validation_unsupportedGlobChar() {
        val toml = """
            [g]
            log.files.pattern = "app.?.log"
        """.trimIndent()

        MoreAssertions.assertThatThrownBy { PollerConfigParser.parse(toml) }
            .isLike(IllegalArgumentException("log.files.pattern only supports '*' as a glob metacharacter, found '?' in 'app.?.log'"))
    }
    ...
    fun testParse_emptyFilesPattern_throwsNonEmptyError() {
        ...
        MoreAssertions.assertThatThrownBy { PollerConfigParser.parse(toml) }
            .isLike(IllegalArgumentException("log.files.pattern must be a non-empty string"))
    }
    ...
    fun testParse_logFiles_onlyPatternProvided_appliesAllDefaults() {
        ...
        assertThat(PollerConfigParser.parse(toml)).isEqualTo(mapOf(
            "myapp" to LogGroupConfig(log = LogSection(files = FilesConfig(pattern = "app.log"))),
        ))
    }
    ...
    fun testParse_logFiles_rootOmitted_rootPathResolvesToCwd() {
        ...
        val config = PollerConfigParser.parse(toml).getValue("g")
    ...
    fun testEffectiveDateField_logFiles_jsonFormatWithoutDateField_defaultsToTimestamp() {
        ...
        val log = PollerConfigParser.parse(toml).getValue("api").log
    ...
    fun testParse_byteSize_suffixToBytesResolution() {
        fun parseSize(value: String): Long {
            ...
            """.trimIndent()
            return PollerConfigParser.parse(toml).getValue("g").log.maxEventByteSize.bytes
        }
    ...
    fun testParse_multipleGroups() {
        ...
        assertThat(PollerConfigParser.parse(toml)).isEqualTo(
            mapOf(
                "myapp" to LogGroupConfig(
                    log = LogSection(
                        files = FilesConfig(root = "/var/log/myapp", pattern = "application.log*"),
                ...
                "nginx" to LogGroupConfig(
                    log = LogSection(
                        files = FilesConfig(root = "/var/log/nginx", pattern = "access.log*"),
